...
public class K_thPosition {
    public static int k_thPositionNumber(int arr[], int k) {
        Set<Integer> allNumber = new HashSet<>();
        for (int i = 1; i <= 1000; i++) {
            allNumber.add(i);
        }
        for (int i = 0; i < arr.length; i++) {
            if (allNumber.contains(arr[i])) {
                allNumber.remove(arr[i]);
            }
        }
        ArrayList<Integer> integers = new ArrayList<>(allNumber);
